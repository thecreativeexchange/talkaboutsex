...
BaArchive *ShKeyboard::m_pArchive = BtNull;
RsMaterial *ShKeyboard::m_pKeyboard = BtNull;
...
//static
void ShKeyboard::Setup( BaArchive *pArchive )
{
	m_pArchive = pArchive;
	m_pKeyboard = pArchive->GetMaterial( "keyboard" );
}
...
//static
void ShKeyboard::Render()
{
	if( m_isVisible == BtTrue )
	{
		MtVector2 v2KeyboardDimension = m_pKeyboard->GetDimension();
		MtVector2 v2ScreenDimension = RsUtil::GetDimension();
		BtFloat aspect = v2KeyboardDimension.y / v2KeyboardDimension.x;
		MtVector2 v2Dimension( v2ScreenDimension.x, v2ScreenDimension.x * aspect );
		MtVector2 v2Position( 0, 0 );
		v2Position.y = RsUtil::GetHeight() - v2Dimension.y;
		m_pKeyboard->Render( v2Position, v2Dimension, RsColour::WhiteColour(), MaxSortOrders - 1 );
	}
}
