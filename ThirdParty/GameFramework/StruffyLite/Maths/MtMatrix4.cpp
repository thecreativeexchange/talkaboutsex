...
MtMatrix4 MtMatrix4::GetInverse() const
{
    ...
    for (i=0; i<4; i++) {               /* eliminate in column i, below diag */
        ...
        for (k=i; k<4; k++)             /* find pivot for column i */
            if (fabs(A(k, i)) > max) {
                max = (BtFloat) fabs(A(k, i));
                j = k;
            }
        if (max<=0.) return MtMatrix4();         /* if no nonzero pivot, PUNT */
