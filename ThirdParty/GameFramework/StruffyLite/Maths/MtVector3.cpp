...
{
    BtBool t = (
       (fabs(myvector.x - x) < MtTol ) &&
       (fabs(myvector.y - y) < MtTol ) &&
       (fabs(myvector.z - z) < MtTol )
           );
...
MtVector3 MtVector3::GetNormalize()
{
   MtVector3 result;
...
MtVector3 MtVector3::GetNormalise()
{
   return GetNormalize();
}

MtVector3 MtVector3::CrossProduct (const MtVector3& v2)
{
    MtVector3 result;
