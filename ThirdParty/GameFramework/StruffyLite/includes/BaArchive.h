...
struct LBaArchiveEntry
{
	BtChar						m_szTitle[LMaxArchiveName];
	BtU32						m_nArchiveSize;
	BtU32						m_nInstanceSize;
	BtU32						m_nResources;
};

struct LBaArchiveHeader
{
	BtChar						m_szTitle[LMaxArchiveName];
...
private:
	...
	BtU8*						m_pMemory;
	BtU8*						m_pMemoryArchiveDictionary;
	BtU32						m_nArchives;
	LBaArchiveEntry*			m_pArchiveDictionary;
