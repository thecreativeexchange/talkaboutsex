...
RsTexture* RsTextureWinGL::GetDuplicate() const
{
    return BtNull;
}
...
void RsTextureWinGL::CreateOnDevice()
{
    ...
    
    {
		...
		if( m_pFileData->m_flags & RsTF_RenderTarget )
		{
            ...
            glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24_OES, mipMap.m_nWidth, mipMap.m_nHeight );
		...
		else
		{
            ...
            if( ( m_pFileData->m_flags & RsTF_Alpha ) == 0 )
            {
                format = GL_RGB;
            }
