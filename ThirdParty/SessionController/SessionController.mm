...
{
    ShNetworkAction alert;
    strcpy( alert.m_peerID, [[peerID displayName] UTF8String] );
    alert.m_type = ShNetworkActionType_Received;
    alert.m_size = (int)[data length];
    memcpy( alert.m_buffer, data.bytes, alert.m_size );
    ShNetwork::PushAlert(alert);
}
