...
inline bool GetBoolFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, bool &num)
{
	// Find the span
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
	{
		const char *pNumAsString = pNum->GetText();
		num = false;
		if( atoi(pNumAsString) )
		{
			num = true;
		}
	}
...
inline unsigned int GetUIntFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, unsigned int &num)
{
	// Find the span
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
...
inline int GetIntFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, int &num)
{
	// Find the span
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
...
inline float GetFloatFromXML(tinyxml2::XMLNode *pItem, const char *pItemString )
{
	float num = 0;
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
...
inline bool GetStringFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, float *pValues )
{
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
	{
		...
		sscanf( pNumAsString, "%f %f %f", &pValues[0], &pValues[1], &pValues[2] );
		return true;
	}
	return false;
}
...
inline void GetStringFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, char *dest )
{
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
	{
		strcpy( dest, pNum->GetText() );
	}
}
...
inline float GetFloatFromXML(tinyxml2::XMLNode *pItem, const char *pItemString, float &num )
{
	tinyxml2::XMLElement *pNum = pItem->FirstChildElement(pItemString);
	if(pNum)
	{
...
inline float *GetVectorFromXML(tinyxml2::XMLNode *pItem, float *floats)
{
	GetFloatFromXML(pItem, "X", floats[0]);
	GetFloatFromXML(pItem, "Y", floats[1]);
	GetFloatFromXML(pItem, "Z", floats[2]);
	return floats;
}
